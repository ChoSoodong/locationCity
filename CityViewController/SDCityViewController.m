...
#import "SDCityHeaderView.h"
#import "SDAreaDataManager.h"
#import "SDLocation.h"
#import "SDSearchView.h"
...
SDLocationDelegate,
SDCityHeaderViewDelegate,
SDSearchViewDelegate>
...
@property (nonatomic, strong) SDCityTableViewCell *cell;
@property (nonatomic, strong) SDCityHeaderView *headerView;
@property (nonatomic, strong) SDAreaDataManager *manager;
@property (nonatomic, strong) SDLocation *locationManager;
@property (nonatomic, strong) SDSearchView *searchView;
/** 最近访问的城市*/
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(chooseCityWithName:) name:SDCityTableViewCellDidChangeCityNotification object:nil];
        ...
        dispatch_async(serialQueue, ^{
            [self processData:^(id success) {
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    [_rootTableView reloadData];
                    self.locationManager = [[SDLocation alloc] init];
...
- (void)initWithJFAreaDataManaager {
    _manager = [SDAreaDataManager shareInstance];
...
- (SDCityHeaderView *)headerView {
    if (!_headerView) {
        _headerView = [[SDCityHeaderView alloc] initWithFrame:CGRectMake(0, 0, self.view.bounds.size.width, KHeaderViewHeight)];
...
- (SDSearchView *)searchView {
    if (!_searchView) {
        _searchView = [[SDSearchView alloc] initWithFrame:CGRectMake(0, NAVIGATION_HEIGHT+KSearchHeight, Kscreen_width, Kscreen_height  - (NAVIGATION_HEIGHT+KSearchHeight) - BOTTOM_BAR_HEIGHT)];
        _searchView.backgroundColor = RGBAColor(155, 155, 155, 0.5);
...
- (UITableView *)rootTableView {
    if (!_rootTableView) {
        _rootTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, Kscreen_width, Kscreen_height-BOTTOM_BAR_HEIGHT) style:UITableViewStylePlain];
        _rootTableView.delegate = self;
        _rootTableView.dataSource = self;
        _rootTableView.sectionIndexColor = RGBAColor(0, 132, 255, 1);
        [_rootTableView registerClass:[SDCityTableViewCell class] forCellReuseIdentifier:@"cityCell"];
